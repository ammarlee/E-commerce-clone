...
<div class="add-product text-center text-capitalize ">
  <h1 class="font-weight-bold">products page
  </h1>
  <div class="mt-15">
...
import { VueEditor } from "vue2-editor";
  export default {
   ...
   async mounted() {
     this.overlay = true
     try {
    const res = await Functions.fetchUserProducts(this.$store.getters.getUser)
     this.overlay = false
    ...
    methods: {
      ...
      Functions.editProduct(formData)
        ...
        this.editmode =false
          ...
          this.overlay = false
          this.dialogNotifyError(err.response.data.msg)
      ...
      handleSubmit() {
       this.overlay = true
         ...
         axios.post('https://full-meven-stack.herokuapp.com/file-upload',formData,{
            ...
            this.dialog= false
            this.overlay = false
           this.alertDisplay('success','added new product',2000)
          }).catch(err=>{
          this.dialogNotifyError(err.response.data.msg)
            this.overlay = false
            this.errors = err
