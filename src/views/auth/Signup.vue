  ...
  <v-form
      ...
      <v-window v-model="step">
        <v-window-item :value="1">
          ...
          have an account? <router-link tag="a" to="/login">login</router-link>
            </span>
          </v-card-text>
        </v-window-item>
...
export default {
  ...
  computed:{
     currentTitle () {
        switch (this.step) {
          ...
          case 2: return 'Please your information'
  ...
  methods: {
   ...
   async addNewUser(){
     try {
     let newUser = await Functions.singup({...this.user})
     const token = newUser.data.token
     this.$store.dispatch('setUser',newUser.data.newuser)
      this.$store.dispatch('setAuth',newUser.data.authanticated)
      this.$store.dispatch('setToken',token)
    this.alertDisplay("success",'signup succefully',2000) 
    this.$router.push('/')
