<template>
  <div>  
    <v-toolbar color="orange accent-1 " >
      <v-app-bar-nav-icon style="cursor: pointer;"  class="pt-2" @click="openDrawer"></v-app-bar-nav-icon>
     ...
     <!-- THE SEARCHING BAR  -->
      ...
      <v-toolbar-title class="title ml-1 d-none  d-sm-inline" style="height:67px" >
          ...
          <v-card>
            ...
            <v-list-item-content class="justify-center">
              <div class="mx-auto text-center">
                ...
                <router-link v-for="item in menuItems" :key="item.id" :to="item.link" tag="span" class="pt- headline d-md-none">
                  <v-btn text> <v-icon>{{item.icon}}</v-icon>{{item.page}} </v-btn>
                <v-divider class="my-3"></v-divider>
                </router-link>
                <v-btn
                  depressed
...
import searchingBar from './searchingBar'
  export default {
    ...
    openDrawer(){
     ...
     computed: {
      ...mapGetters(['getCategory','getUser','getTotalPrice','isLoggedIn','token','notifications']),
